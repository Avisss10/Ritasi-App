...
const namaGalianInput = document.getElementById('namaGalian');
const hargaGalianInput = document.getElementById('hargaGalian');
const btnTambah = document.querySelector('.header .btn-primary');
const btnSubmit = document.getElementById('btnSubmit');
const tableBody = document.getElementById('tableBody');
const searchInput = document.getElementById('searchInput');
const alertContainer = document.getElementById('alertContainer');
const deleteGalianName = document.getElementById('deleteGalianName');

// ============================================================================
// EVENT LISTENERS
// ============================================================================

// Auto format harga galian saat pengetikan
hargaGalianInput.addEventListener('input', (e) => {
    let value = e.target.value.replace(/[^\d]/g, ''); // Hapus non-numeric
    
    if (value) {
        // Format dengan separator ribuan, tanpa .00
...
function openAddModal() {
    currentEditId = null;
    modalTitle.textContent = 'Tambah Galian';
    namaGalianInput.value = '';
    hargaGalianInput.value = '';
    btnSubmit.textContent = 'Simpan';
    openModal('modalForm');
    namaGalianInput.focus();
}
...
function openEditModal(id) {
    ...
    if (data) {
        modalTitle.textContent = 'Edit Galian';
        namaGalianInput.value = data.nama_galian;
        // Format harga yang ada dengan pemisah ribuan
        if (data.harga_galian) {
            hargaGalianInput.value = new Intl.NumberFormat('id-ID').format(data.harga_galian);
        } else {
            hargaGalianInput.value = '';
        }
...
function openDeleteModal(id) {
    ...
    if (data) {
        deleteGalianName.textContent = data.nama_galian;
...
async function handleSubmit(e) {
    ...
    const namaGalian = namaGalianInput.value.trim();
    const hargaGalianFormatted = hargaGalianInput.value;
    
    // Extract numeric value dari formatted input
    const hargaGalian = hargaGalianFormatted ? parseInt(hargaGalianFormatted.replace(/[^\d]/g, '')) : '';

    if (!namaGalian) {
        showAlert('Nama galian tidak boleh kosong', 'error');
        return;
    }

    try {
        let response;
        const payload = { nama_galian: namaGalian };
        if (hargaGalian) payload.harga_galian = hargaGalian;
...
function renderTable() {
        ...
        tableBody.innerHTML = `
            <tr>
                <td colspan="4" class="empty-state">
                    <p>📋 Belum ada data galian</p>
                    <button class="btn btn-primary" onclick="openAddModal()">+ Tambah Galian Pertama</button>
                </td>
            </tr>
        `;
        return;
    ...
    tableBody.innerHTML = filteredData
        .map((item, index) => `
            <tr>
                <td>${index + 1}</td>
                <td>${escapeHtml(item.nama_galian)}</td>
                <td>Rp ${formatNumber(item.harga_galian || 0)}</td>
                <td>
...
function filterTable() {
    ...
    if (!query) {
        ...
        filteredData = allData.filter(item =>
            item.nama_galian.toLowerCase().includes(query)
        );
    }
...
function showLoadingState() {
    tableBody.innerHTML = `
        <tr>
            <td colspan="4" class="loading-row">Memuat data...</td>
        </tr>
    `;
}
